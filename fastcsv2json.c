...
size_t MAX_COLUMNS = COLUMNS_DEFAULT, MAX_COLUMN_SIZE = COLUMN_SIZE_DEFAULT;
...
{
  static char *start = NULL;	/* stores string str for
				 * consecutive calls */
...
generate_json ()
{
  ...
  int counter,
    ncolumns = 0, len = 0, nlines = 0, result = 1, lisize = 0, losize = 0;
  ...
  for (counter = 0; counter < MAX_COLUMNS; counter++)
    {
      column_name[counter] =
	(char *) calloc (MAX_COLUMN_SIZE, sizeof (char *));
   ...
   */
  lisize = MAX_COLUMNS * (MAX_COLUMN_SIZE + 1) + 3;	/* increase column  size by
							   delimiter size plus 3 
							   bytes for cr, lf and null 
							 */
...
parse_arguments (int argc, char *argv[])
{
  ...
  while (counter < argc)
	...
	{
	  fprintf (stderr, "fastcsv2json %d.%d.%d\n",
	  ...
	  fprintf (stderr, "Written by Lucas Tsatiris.\n");
	  return 1;
	}
      else
	/*
	    ...
	    strcmp (argv[counter], "--delimiter") == 0)
	{
	  ...
	  if (counter < argc)
		...
		delimiter[0] = '|';
	      else if (strcmp (argv[counter], "comma") == 0)
		delimiter[0] = ',';
	      else if (strcmp (argv[counter], "semicolumn") == 0)
		delimiter[0] = ';';
	      else if (strcmp (argv[counter], "column") == 0)
		delimiter[0] = ':';
	      else if (strcmp (argv[counter], "tab") == 0)
		delimiter[0] = 9;
	      else
		{
